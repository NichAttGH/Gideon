    ...
    def generate_plans(self, problems_paths, domain_path, plans_dir_path, domain, results_dir_path):
        ...
        try:
            if self.num_problems > 1:
                ...
                p_failed = 0    # Number of plans failed
                ...
                if ppg != 0:
                    ...
                    with tqdm(initial=ppg, total=self.num_problems, leave=True, desc="Generating plans", unit=" plan") as pbar:
                        ...
                        for i, problem in enumerate(problems_paths[ppg:], start=ppg):
                            p_failed, new_name, execution_times = choose_planner(
                                self.planner, self.planner_path, domain_path, problem, plans_dir_path,
                                self.output_dir, domain, i, p_failed, execution_times)
                            p_failed = check_val_and_move(
                                self.validate_path, domain_path, problem, new_name, plans_dir_path, p_failed)
                ...
                delete_and_renumber(self.output_dir, failed_problems)
                ...
                self.planner_logs_manager.generate_log_file(
                    i+1, p_failed, plans_dir_path, t, avg_time, min_time, max_time, median_time, std_dev)
                
                # Final output for the user
                final_output(i+1, p_failed, plans_dir_path, t, avg_time, min_time, max_time, median_time, std_dev)
            ...
            while True:
                ...
                if user_input == 'y':
                    ...
                    # Generate log file
                    self.planner_logs_manager.generate_log_file(
                        i, p_failed, plans_dir_path, t, avg_time, min_time, max_time, median_time, std_dev)
                    
                    # Final output for the user
                    final_output(i, p_failed, plans_dir_path, t, avg_time, min_time, max_time, median_time, std_dev)
                    break
                else:
...
def choose_planner(planner, planner_path, domain_path, problem_path, plans_dir_path, output_dir, domain, i, p_failed, execution_times):
    """
    ...
    Returns:
        tuple: (p_failed, new_name, execution_times)
    """
    #global stop_event  # Accesso alla variabile globale stop_event
    ...
    if planner == "probe":
        try:
            ...
            return p_failed, new_name, execution_times
        except Exception as e:
            print(f"Error while executing planner: {e}")
            p_failed += 1  # Increment p_failed in case of error
            return p_failed, None, execution_times
...
def check_val_and_move(validate_path, domain_path, problem_path, new_name, plans_dir_path, p_failed):
    """
