...
# Module imports
from Hash_list_manager import generate_hash, update_h_progress
...
class PDDL_Generator:
    ...
    """
    def __init__(self, num_problems, progress_manager, log_manager, hashlist_manager):
        self.num_problems = num_problems
        self.progress_manager = progress_manager
        self.log_manager = log_manager
        self.hashlist_manager = hashlist_manager
    
    def generate_problems(self, generator_path, domain_origin, domain, problems_path, output_path, json_schema, progress_path):
        """
        ...
        Parameters:
            ...
            - progress_path (str): The path for progress file.
        """
        if self.num_problems > 1:
            ...
            if npg != 0:
                ...
                hash_list = self.hashlist_manager.read_hash_list_to_list(answer)  # Needed to update hash list if you want to add more problems
                h_progress = 0      # Progress value for the hash list
            ...
            h_progress = len(hash_list)                     # Progress value of hash_list
            ...
            update_h_progress(progress_path, h_progress)        # Update hash list progress
...
def generate_json_predicates(dict_ordered_by_key, set_predicates):
    ...
    Returns:
        predicates_dict (dict): A dictionary of predicates organized by their respective keys.
    """
...
def generate_constant_initial_state(all_created_objects, json_schema):
    ...
    Returns:
        const_init_state (list): A list of predicates representing the constant initial state.
    """
...
def generate_init_state(predicates_dict, constant_initial_state, json_schema, counts_for_init_state):
    ...
    Returns:
        initial_state (set): A set representing the initial state of the problem.
        counts_for_init_state (dict): The updated counts_for_init_state dictionary reflecting the number of predicates chosen from each pool.
        functions_init (list): A list of functions that will be inside the InitialState.
    """
    ...
    if mutex_pools:
        for i, pool in enumerate(mutex_pools):
            ...
            if not (0.99 <= sum(probabilities) <= 1.01):
                raise ValueError(f"The probabilities for the mutex group {i} do not sum to 1.")
    ...
    functions_init = list()
    for function_name, function_value in json_schema.init_state.functions.items():
        initial_state.add(EqualTo(NumericFunction(function_name), NumericValue(function_value)))
        functions_init.append(function_name)
        
    return initial_state, counts_for_init_state, functions_init
...
def generate_constant_goal_state(all_created_objects, json_schema):
    ...
    Returns:
        const_goal_state (list): A list of predicates representing the constant goal state.
    """
    # Create a dictionary for quick access to Constant objects
    constant_dict = {const.name: const for const in all_created_objects}
...
def generate_goal_state(predicates_dict, constant_goal_state, json_schema, counts_for_goal_state):
    ...
    Returns:
        goal_state (set): A set representing the goal state of the problem.
        counts_for_goal_state (dict): The updated counts_for_goal_state dictionary reflecting the number of predicates chosen from each pool.
        functions_goal (list): A list of functions that will be inside the GoalState.
    """
    ...
    if mutex_pools:
        for i, pool in enumerate(mutex_pools):
            ...
            # Add predicates from the selected pool to the goal state
            for key, value_dict in predicates_dict.items():
                if selected_pool == key:
                    for predicate_list in value_dict.values():
                        for predicate in predicate_list:
                ...
                for predicate_list in value_dict.values():
                    for predicate_structure in predicate_list:
                        ...
                        if isinstance(predicate_structure, PredicateStructure):
                            if random.random() > predicate_structure.probability:
                                continue  # Skip this predicate based on its probability
                        goal_state.add(predicate_structure)  # Add the predicate to the goal state

    # Extract all functions
    functions_goal = list()
    for function_name, function_value in json_schema.goal_state.functions.items():
        goal_state.add(EqualTo(NumericFunction(function_name), NumericValue(function_value)))
        functions_goal.append(function_name)

    return goal_state, counts_for_goal_state, functions_goal

def generate_metric(json_schema, functions):
    """
    Inspect a JSON-based schema for a metric definition and return
    a corresponding Metric object if the specified function is valid
    and the optimization direction is recognized.

    Parameters:
    - json_schema : An object representing the JSON Schema
    - functions : A list of function names that can be used as metric.

    Returns:
    - A Metric configured for minimization.
    - A Metric configured for maximization.
    """

    metric_optimization = json_schema.metric.get("optimize")
    metric_function = json_schema.metric.get("function")

    if metric_function in functions and metric_optimization.lower() == "minimize":
        return Metric(NumericFunction(metric_function), Metric.MINIMIZE)
    elif metric_function in functions and metric_optimization.lower() == "maximize":
        return Metric(NumericFunction(metric_function), Metric.MAXIMIZE)
...
def generate_single_problem(domain, problem_name, json_schema, counts_for_init_state, counts_for_goal_state):
    ...
    Returns:
        problem (object): The generated PDDL problem instance.
        counts_for_init_state (dict): The updated counts_for_init_state dictionary reflecting the number of predicates chosen from each pool.
        counts_for_goal_state (dict): The updated counts_for_goal_state dictionary reflecting the number of predicates chosen from each pool.
    """
    ...
    init_state, counts_for_init_state, functions_init = generate_init_state(predicates_dict, constant_initial_state, json_schema, counts_for_init_state)
    constant_goal_state = generate_constant_goal_state(all_created_objects, json_schema)
    goal_state, counts_for_goal_state, functions_goal = generate_goal_state(predicates_dict, constant_goal_state, json_schema, counts_for_goal_state)

    metric = generate_metric(json_schema, functions_init)
